...
if not raidOverride:
    ...
    diskListRe = re.findall(r'^/dev/(\w+)', allDisksStdout, re.M)
else:
    diskListRe = raidOverride

for d in diskListRe:
    ctlOut = subprocess.getoutput('smartctl -a /dev/' + d)

    if raidOverride:
        d = d.replace(',', '').replace(' -d', '').replace('[', '').replace(']', '').replace('+', '_').replace(' ', '_')  # slightly sanitize the item key. for RAID only
    #print('disk:                         ', d)
